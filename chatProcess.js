window.onload = populate_country_code();
...
function populate_country_code() {
	...
	xobj.onreadystatechange = function () {
		if (xobj.readyState == 4 && xobj.status == "200") {
			...
			let listcodes = JSON.parse(xobj.responseText);
			...
			for (let i = 0; i < listcodes.length; i++) {
				...
				let select = document.messageForm.dialcode;
...
function send() {
	let dialcode = document.messageForm.dialcode.value;
	let phone = dialcode + document.messageForm.phone.value;
	let message = encodeURI(document.messageForm.message.value); // need to encode the text
